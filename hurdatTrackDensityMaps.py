...
interval = 1
dataset = xr.open_dataset(os.path.join(DATA_DIR, 'HURDAT2DensityALL2025.nc'))
RI = dataset['RI']#xr.open_dataset(r"C:\Users\deela\Downloads\RIDensity.nc")
track = dataset['track']#xr.open_dataset(r"C:\Users\deela\Downloads\trackDensity.nc")
change = dataset['24hrChange']#xr.open_dataset(r"C:\Users\deela\Downloads\\24hrChangeDensity.nc")
ACE = dataset['ACE']#xr.open_dataset(r"C:\Users\deela\Downloads\ACEDensity.nc")
wind = dataset['wind']#xr.open_dataset(r"C:\Users\deela\Downloads\windDensity.nc")
...
def makePlot(years, dataType = 'track'):
    ...
    ax.set_title(f'{interval}\u00b0x{interval}\u00b0 (Gaussian Filter, σ = 3)\nDeelan Jariwala', fontsize=9, loc='right') 
    cbar = plt.colorbar(c, orientation = 'vertical', aspect = 50, pad = .02)
    cbar.ax.tick_params(axis='both', labelsize=9, left = False, bottom = False)
    cbar.set_ticks(np.arange(vmin, vmax + ((vmax - vmin) / 20), (vmax - vmin) / 10))
    plt.savefig(os.path.join(OUTPUTS, 'hurdatDensityPlot.png'), dpi = 400, bbox_inches = 'tight')
    plt.close()

#makePlot([1969, 2023], 'track')