...
def ships(storm, year, mon, day, hr = None):
    try:
        ...
        link = f'https://ftp.nhc.noaa.gov/atcf/archive/MESSAGES/20{year}/stext/{year}{str(mon).zfill(2)}{str(day).zfill(2)}{str(hr).zfill(2)}{storm.upper()}{year}_ships.txt'
        data = urllib.urlopen(link).read().decode('utf-8')
        stime = f'{mon}/{day}/20{year} at {hr}z'
    print(stime)
