...
USAGE = f'```$shart [storm] [year] [month] [day] [optional: hour]```'

with open(os.path.join(DATA_DIR, 'SHIPS_RF_RI.cpickle'), 'rb') as f:
    regr = pickle.load(f)

def plot(storm, stime, SST, VMax, RH, DSHR, DDIR, MPI, LAT, SLAT, CLAT, RH24, VMAXd12, SHR24, DIST12, DIST24, DIV, T200, OHC, TADV, convectionArea):
       feature = np.array([SST, SST / OHC, VMax, RH, DSHR, DDIR, VMax - MPI, SLAT, CLAT, RH24, VMAXd12, SHR24, DIST12, DIST24, DIV, T200, OHC, TADV, convectionArea, DSHR * SST, convectionArea * DSHR, (SST - T200) / MPI, ((100 - RH) * DSHR) / MPI])
       ...
       plt.title(f'SHIPS-Based Random Forest 24hr Intensity Prediction: {storm.upper()}\nNumber of Predictions: {len(predArray)}' , fontweight='bold', fontsize=8 + 1, loc='left')
       ...
       plt.savefig(os.path.join(OUTPUTS, 'SHART.png'), dpi = 400, bbox_inches = 'tight')
       plt.legend(loc = 'upper right')
       plt.close()


def getShips(storm, year, month, day, hour = None):
    try:
          line, stime, ri, an, link, VMAXd12, convectionArea = ships.ships(storm.upper(), str(year)[2:4], str(month).zfill(2), str(day).zfill(2), str(hour).zfill(2))
    except:
          line, stime, ri, an, link, VMAXd12, convectionArea = ships.ships(storm.upper(), str(year)[2:4], str(month).zfill(2), str(day).zfill(2))

    SST = float(line[6, 1])
    VMax = float(line[1, 1])
    RH = float(line[11, 1])
    DSHR = float(line[3, 1])
    DDIR = (float(line[5, 1]) + 180.0) % 360.0
    MPI = float(line[7, 1])
    lat = float(line[17, 1])
    ...
    plot(storm, stime, SST, VMax, RH, DSHR, DDIR, MPI, lat, SLAT, CLAT, RH24, VMAXd12, SHR24, DIST12, DIST24, DIV, T200, OHC, TADV, convectionArea)
    
    return link 
# getShips('al13', '2025', '10', '24')